...
func FixedBundledAudioCommand(audioPath, stickerName string, stickerStore *sticker.Store) func(ctx context.Context, client *whatsmeow.Client, evt *events.Message, args []string) error {
	return func(ctx context.Context, client *whatsmeow.Client, evt *events.Message, args []string) error {
		if err := whatsapp.SendBundledOggVoice(ctx, client, evt, audioPath); err != nil {
			return err
		}
		if stickerName != "" {
			_ = sticker.Send(ctx, client, evt, stickerName, false, stickerStore)
		}
		return nil
	}
}
