...
// IntensityLabel devolve o rótulo em português para feedback ao utilizador.
func IntensityLabel(i Intensity) string {
	switch i {
	case IntensityLight:
		return "leve"
	case IntensityHeavy:
...
func Apply(ctx context.Context, data []byte, ext string, effectName string, intensity Intensity) ([]byte, error) {
	...
	// Garante índice válido no array de filtros (leve / médio / forte).
	idx := int(intensity)
	if idx < 0 || idx > 2 {
		idx = int(IntensityMedium)
	}
	filter := effect.Filters[idx]
